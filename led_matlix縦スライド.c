...
{0x04,0x00,0x0C,0x10,0x08,0x18,0x1F,0xE8,0x02,0x00,0xFF,0xFF,0x04,0x20,0x0C,0x30,0x39,0x9C,0xE3,0x07,0x0E,0x60,0x38,0xC0,0x07,0x8C,0x3C,0x38,0x01,0xE0,0x3F,0x00},
{0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0xFC,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0xFF,0xFF,0x00,0x00},
{0x00,0x00,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x01,0x80,0x01,0x80,0x00,0x00,0x00,0x00},
{0x00,0x00,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x80,0x01,0x00,0x01,0x00,0x01,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x01,0x80,0x01,0x80,0x00,0x00,0x00,0x00}
};
...
int main (void){
	...
	while(1){
		for(i = 0; i < COL; i++){
			...
			/*if(moveShakeCount <= 63)
				buf = buf  << moveShakeCount;
			
			else if(moveShakeCount >= 64 && moveShakeCount <= 128)
				buf = */
			
			
			//横スライドでオーバーフローした部分を付け足す	表示bit数+表示空白数
			//buf |= ledData[i] >> ((DISPLINE + DISPBLANK) - moveShakeCount);
			
			//行の決定(これをforの最後に持っていくと、初期値が15になる)
			GPIOA->ODR = i + moveShakeCount;
		...
		if(moveShakeCount == (DISPLINE + DISPBLANK)){
			moveShakeCount = 0;
		}
		//moveShakeCount++;
	}
